...
public class AdminRoleServiceImpl extends BaseServiceImpl<AdminRoleMapper, AdminRole> implements IAdminRoleService {
    ...
    public JSONObject auth(Long userId) {
        ...
        // --- OceanEngine module injection ---
        // Step 1: For super admin, bypass all menu filters and build oceanengine auth directly from DB
        if (UserUtil.isAdmin() && !jsonObject.containsKey("oceanengine")) {
            AdminMenu oeRoot = adminMenuService.lambdaQuery()
                    .eq(AdminMenu::getRealm, "oceanengine")
                    .eq(AdminMenu::getStatus, 1)
                    .last("limit 1")
                    .one();
            if (oeRoot != null) {
                List<AdminMenu> children = adminMenuService.lambdaQuery()
                        .eq(AdminMenu::getParentId, oeRoot.getMenuId())
                        .eq(AdminMenu::getStatus, 1)
                        .list();
                JSONObject oeJson = new JSONObject();
                for (AdminMenu child : children) {
                    if (StrUtil.isNotEmpty(child.getRealm())) {
                        oeJson.put(child.getRealm(), true);
                    }
                }
                jsonObject.put("oceanengine", oeJson);
            }
        }
        // Step 2: For non-admin users, keep queryMenuList-based behavior
        if (!UserUtil.isAdmin() && !jsonObject.containsKey("oceanengine")) {
            List<AdminMenu> menuList = adminMenuService.queryMenuList(userId);
            boolean hasOeMenu = menuList.stream()
                    .anyMatch(menu -> StrUtil.isNotEmpty(menu.getRealm()) && menu.getRealm().startsWith("oceanengine"));
            if (hasOeMenu) {
                Set<AdminMenu> oeMenus = menuList.stream()
                        .filter(menu -> StrUtil.isNotEmpty(menu.getRealm()) && menu.getRealm().startsWith("oceanengine"))
                        .collect(Collectors.toSet());
                JSONObject oeAuth = createMenu(oeMenus, 0);
                if (!oeAuth.isEmpty()) {
                    jsonObject.put("oceanengine", oeAuth);
                }
            }
        }
        // P0 fallback: ensure OceanEngine module is visible for super admin
        if (UserUtil.isAdmin() && !jsonObject.containsKey("oceanengine")) {
            boolean hasOeRoot = adminMenuService.lambdaQuery()
                    .eq(AdminMenu::getRealm, "oceanengine")
                    .eq(AdminMenu::getStatus, 1)
                    .last("limit 1")
                    .one() != null;
            if (hasOeRoot) {
                // For permission.js, it's enough that "oceanengine" exists as a non-null object.
                jsonObject.put("oceanengine", new JSONObject());
            }
        }
