...
public class AdminUserServiceImpl extends BaseServiceImpl<AdminUserMapper, AdminUser> implements IAdminUserService {
    ...
    public BasePage<AdminUserVO> queryUserList(AdminUserBO adminUserBO) {
        if (adminUserBO == null) {
            ...
            List<AdminUserVO> adminUserVOList = this.lambdaQuery().ne(AdminUser::getUserId, UserUtil.getSuperUser())
                    .list().stream().map(adminUser -> this.convertAdminUserToVo(adminUser, 2))
        ...
        List<Long> longs = ApplicationContextHolder.getBean(IAdminUserHisTableService.class).listObjs(queryWrapper,
                user -> Long.valueOf(user.toString()));
        basePage.getRecords().forEach(adminUserVO -> {
            ...
            adminUserVO.setRoleId(adminRoleList.stream().map(adminRole -> adminRole.getRoleId().toString())
                    .collect(Collectors.joining(",")));
            adminUserVO
                    .setRoleName(adminRoleList.stream().map(AdminRole::getRoleName).collect(Collectors.joining(",")));
        });
        return basePage;
    }
    ...
    public JSONObject countUserByLabel() {
        ...
        jsonObject.fluentPut("allUserCount", getBaseMapper().countUserByLabel(0, null))
    ...
    public void setUser(AdminUserVO adminUserVO) {
        ...
        // 用户姓名处理
        Integer nameCount = query().eq("realname", adminUserVO.getRealname()).ne("user_id", adminUserVO.getUserId())
                .count();
        if (nameCount > 0) {
        ...
        adminUserRoleService.saveByUserId(adminUserVO.getUserId(), true,
                StrUtil.splitTrim(adminUserVO.getRoleId(), Const.SEPARATOR));
    ...
    public void addUser(AdminUserVO adminUser) {
        ...
        adminUserRoleService.saveByUserId(adminUserPO.getUserId(), false,
                StrUtil.splitTrim(adminUser.getRoleId(), Const.SEPARATOR));
    ...
    public void updateUser(AdminUser adminUser) {
        ...
        if (StrUtil.isNotEmpty(adminUser.getPassword())) {
            ...
            adminUser.setPassword(UserUtil.sign((adminUser.getUsername().trim() + adminUser.getPassword().trim()),
                    adminUser.getSalt()));
        }
        ...
        ExcelUtil.readBySax(filePath, 0, (int sheetIndex, int rowIndex, List<Object> rowList) -> {
            if (rowIndex > 1) {
                ...
                if (!StrUtil.isEmpty(parentName)) {
                    AdminUser user = lambdaQuery().select(AdminUser::getUserId).eq(AdminUser::getRealname, parentName)
                            .last("limit 1").one();
                    if (user == null) {
    ...
    private String getFilePath(MultipartFile file) {
        ...
        try {
            ...
            File fromStream = FileUtil.writeFromStream(inputStream,
                    dirPath + "/" + IdUtil.simpleUUID() + file.getOriginalFilename());
    ...
    public void setUserStatus(AdminUserStatusBO adminUserStatusBO) {
        for (Long id : adminUserStatusBO.getIds()) {
            ...
            lambdaUpdate().set(AdminUser::getStatus, adminUserStatusBO.getStatus()).eq(AdminUser::getUserId, id)
                    .update();
        }
    }
    ...
    public void resetPassword(AdminUserStatusBO adminUserStatusBO) {
        ...
        for (Long id : adminUserStatusBO.getIds()) {
            AdminUser adminUser = getById(id);
            String password = UserUtil.sign(adminUser.getUsername() + adminUserStatusBO.getPassword(),
                    adminUser.getSalt());
    ...
    public List<SimpleUser> queryUserByIds(List<Long> ids) {
        ...
        queryWrapper.select(AdminUser::getUserId, AdminUser::getImg, AdminUser::getRealname)
                .in(AdminUser::getUserId, ids).last(" ORDER BY instr('," + idStr + ",',CONCAT(',',user_id,','))");
    ...
    public List<Long> queryNormalUserByIds(List<Long> ids) {
        ...
        String idStr = ids.stream().map(String::valueOf).collect(Collectors.joining(Const.SEPARATOR));
        queryWrapper.select(AdminUser::getUserId, AdminUser::getImg, AdminUser::getRealname)
                .in(AdminUser::getUserId, ids).eq(AdminUser::getStatus, 1)
    ...
    public DeptUserListVO queryDeptUserList(Integer deptId, boolean isAllUser) {
        ...
        List<DeptVO> collect = deptList.stream().filter(dept -> dept.getPid().equals(deptId))
                .collect(Collectors.toList());
    ...
    public Set<HrmSimpleUserVO> queryDeptUserListByHrm(DeptUserListByHrmBO deptUserListByHrmBO) {
        ...
        if (CollUtil.isNotEmpty(deptUserListByHrmBO.getUserIdList())) {
            List<AdminUser> userList = query().select("user_id", "realname", "img", "sex", "username as mobile", "post")
                    .in("user_id", deptUserListByHrmBO.getUserIdList()).ne("status", 0).list();
            List<HrmSimpleUserVO> hrmSimpleUserVOS = TransferUtil.transferList(userList, HrmSimpleUserVO.class);
            userVOSet.addAll(hrmSimpleUserVOS);
        }
        return userVOSet;
    ...
    private List<AdminUser> findChildUserList(List<Integer> deptIds) {
        ...
        for (Integer deptId : deptIds) {
            List<AdminUser> list = query().select("user_id", "realname", "img", "sex", "username as mobile", "post")
                    .eq("dept_id", deptId).ne("status", 0).list();
            empList.addAll(list);
            List<AdminDept> childList = adminDeptService.lambdaQuery().select(AdminDept::getDeptId)
                    .eq(AdminDept::getPid, deptId).list();
    ...
    public List<Long> queryUserIdByRealName(List<String> realNames) {
        ...
        return lambdaQuery().select(AdminUser::getUserId).in(AdminUser::getRealname, realNames).list().stream()
                .map(AdminUser::getUserId).collect(Collectors.toList());
    }
    ...
    public void initUser(SystemUserBO systemUserBO) {
        ...
        adminUser.setPassword(
                UserUtil.sign(systemUserBO.getUsername() + systemUserBO.getPassword(), adminUser.getSalt()));
        adminUser.setCreateTime(new Date());
        ...
        adminUserConfigService.save(new AdminUserConfig(null, UserUtil.getSuperUser(), 1, "InitUserConfig",
                jsonObject.toJSONString(), "用户信息"));
    }
    ...
    public List<UserInfo> queryAllUserInfoList() {
        ...
        Map<Long, List<AdminUserRole>> longListMap = userRoles.stream()
                .collect(Collectors.groupingBy(AdminUserRole::getUserId));
        List<UserInfo> userInfoList = adminUserList.stream().map(user -> BeanUtil.copyProperties(user, UserInfo.class))
                .collect(Collectors.toList());
