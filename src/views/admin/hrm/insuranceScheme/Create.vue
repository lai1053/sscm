          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <template>{{ getProjectName(scope.row.type) }}</template>
              </template>
            </el-table-column>
            <el-table-column
            ...
            <el-table-column
              ...
              width="100">
              <template slot-scope="scope">
                <el-button type="text" @click="deleteItems(scope.$index, otherFrom.topList)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-dropdown
        ...
        <div class="table-wrap">
          <el-table
            ...
            <el-table-column
              ...
              width="100">
              <template slot-scope="scope">
                <el-button type="text" @click="deleteItems(scope.$index, otherFrom.bottomList)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-dropdown
    ...
    getSchemeTypeDetail(value) {
      hrmInsuranceSchemeScaleAPI({
        ...
        if (trDoms) {
          ...
          const keys = {
            1: 'defaultAmount',
            5: 'corporateProportion',
            6: 'personalProportion',
            7: 'corporateAmount',
            8: 'personalAmount'
          }
          trList.forEach(trItem => {
            if (trItem.children && trItem.children.length) {
              ...
              if (tdDom.nodeName == 'TD') {
                ...
                const sysType = this.getSysSchemeTypeValue(firstTd.innerText)
                if (sysType) {
                  const typeItems = {}
                  for (let index = 0; index < trChildren.length; index++) {
                    const td = trChildren[index]
                    if (keys[index]) {
                      typeItems[keys[index]] = td.innerText.replace('%', '')
                    }
                  }
