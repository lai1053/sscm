  ...
  <div id="employee-main-container" class="candidate-index">
    ...
    >
      <div
        ...
        slot="top"
        style="color: #999; margin-bottom: 10px;">可根据未更改候选人简历状态的时间清理候选人</div>
      <div
        v-if="formAddCommand === 'clear'"
    ...
    statusHandleChange(command) {
      ...
      command == 'new') {
        ...
        this.formAddForm = {
          ...
          otherInterviewEmployeeIds: data ? (data.otherInterviewEmployeeIds ? data.otherInterviewEmployeeIds.split(',').filter(item => !isEmpty(item)).map(item => parseInt(item)) : []) : [],
