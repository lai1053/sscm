...
export default {
  ...
  methods: {
    ...
    formChange(item, index, value, valueList, editForm) {
      if ((this.formAddTitle === '调整部门/岗位' || this.formAddTitle === '晋升/降级') && item.field == 'changeType') {
        if (value == 7) {
      ...
      } else if (this.formAddTitle === '办理离职' && item.field == 'quitType') {
        if (value == 1) {
    ...
    handleTypeClick(command, data) {
      ...
      if (command === 'leave') {
        ...
        this.formAddTitle = command === 'change-post' ? '调整部门/岗位' : '晋升/降级'
        this.formAddForm = {
    ...
    formAddPass() {
      ...
      let request = null
      if (this.formAddTitle === '办理离职') {
        request = hrmEmployeePostAddLeaveAPI
      } else if (this.formAddTitle === '办理转正') {
        request = hrmEmployeeSetBecomeAPI
      } else if (this.formAddTitle === '调整部门/岗位') {
        request = hrmEmployeeSetChangePostAPI
      } else if (this.formAddTitle === '晋升/降级') {
