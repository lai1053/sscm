            ...
            <members-dep
              ...
              <span slot="membersDep">
                <el-tooltip
                  ...
                  <i
                    v-show="(formData.sendUserList.length + (formData.sendDeptList ? formData.sendDeptList.length : 0)) > 5"
...
export default {
  ...
  computed: {
    ...
    showSendUserList() {
      const sendUserList = this.formData.sendUserList || []
      const sendDeptList = this.formData.sendDeptList || []
      return sendUserList.concat(sendDeptList).slice(0, 5)
    },

    sendUserDeptNames() {
      const sendUserName = this.formData.sendUserList && this.formData.sendUserList.length > 0 ? `${this.formData.sendUserList.length}个员工` : ''
      const sendDeptName = this.formData.sendDeptList && this.formData.sendDeptList.length > 0 ? `${this.formData.sendDeptList.length}个部门` : ''
      const names = []
      if (sendUserName) {
  ...
  methods: {
    ...
    sendUserChange(users, depts) {
      this.formData.sendUserList = users
      this.formData.sendDeptList = depts
    },
    // 取消
    handleClose() {
